...
namespace {
	...
	bool bone_ok( const havok::Vec4& p )
	{
...
auto IceBox::rage_bot_run( bool enable ) -> void
{
	...
	for ( int i = 0; i < n; ++i ) {
		...
		auto* const oe = op ? op->entity_decrypt( ) : nullptr;
		auto* const sk = oe ? oe->get_skeleton( ) : nullptr;
		if ( !sk ) continue;
